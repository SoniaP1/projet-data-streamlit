...
with st.sidebar:
    ...
    page = st.radio("Navigation", [
        "🏠 Présentation", "📊 DataViz",
        "📐 Économétrie", "🤖 Machine Learning", "🔮 Simulateur"
...
try:
    ...
    df_temps = pd.read_csv(os.path.join(DATA_DIR, "temps.csv"), sep=";", encoding="utf-8")
    df_ressources = pd.read_csv(os.path.join(DATA_DIR, "ressources.csv"), sep=";", encoding="utf-8")
    DATA_OK = True
    DATA_ERROR = ""
except Exception as e:
    DATA_OK = False
...
# ═══════════════════════════════════════════
    ...
    if viz == "Causes d'intervention":
        ...
        st.markdown('<div class="insight">💡 Panne mécanique ~67% des dossiers, accident ~16%, clés/carburant/crevaison ~12%. Le diesel représente 54% des véhicules assistés.</div>', unsafe_allow_html=True)
        ...
        if TOPS_P:
            ...
            plt.xticks(rotation=35, ha="right"); plt.tight_layout(); st.pyplot(fig)
            st.markdown('<div class="insight">💡 TOP D/R présent dans 92.4% des dossiers — service quasi universel. TOP VR (~10%) est la cible de nos modèles ML.</div>', unsafe_allow_html=True)
        ...
        if "Outil.d.assistance" in df_dossiers.columns and "annee" in df_dossiers.columns:
            ...
            st.markdown('<div class="insight">💡 Higgins passe de 17.7% (2021) à 28% (2022) : +10 pts en un an — déploiement progressif confirmé.</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="insight">💡 Médiane ~188s (3 min), moyenne ~312s (5 min). Forte asymétrie à droite → transformation log nécessaire pour la modélisation.</div>', unsafe_allow_html=True)
        ...
        plt.tight_layout(); st.pyplot(fig)
        st.markdown('<div class="insight">💡 Télétravail : médiane légèrement supérieure (+~20s). Différence significative (p < 0.0001) mais faible en valeur absolue.</div>', unsafe_allow_html=True)
    ...
    if section == "Régression OLS – Durée de traitement":
        ...
        coefs = pd.DataFrame({
            ...
            "Interprétation": [
                ...
                "Higgins réduit la durée de ~22% vs MCS"
            ]
        })
        st.dataframe(coefs, use_container_width=True, hide_index=True)
        st.markdown('<div class="insight">⚠️ R² = 5.2% : modèle significatif mais variance non expliquée importante. Hétéroscédasticité détectée → un GLM Gamma serait théoriquement plus adapté.</div>', unsafe_allow_html=True)
        ...
        odds = pd.DataFrame({
            ...
            "Odds Ratio":   [12.53, 10.00, 7.62, 0.37, 0.43, 1.10],
            "Sig. (5%)":    ["✅","✅","✅","✅","✅","✅"],
            "Interprétation": [
                "Multiplie la proba de D/R × 12.5",
                "× 10 — panne nécessite quasi toujours intervention physique",
                "× 7.6 — certains accidents permettent de rouler",
                "÷ 2.7 — remboursements différés sans intervention immédiate",
                "Chaque service supp. réduit la proba de D/R",
                "Higgins légèrement associé à plus de D/R (+10%)"
            ]
        })
